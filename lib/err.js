...
const parseError = (err) => {
  ...
  if (
    ...
    throw new TransactionError(error.data.message, error.data);
  if (rc === "40" || rc === "42" || rc === "43" || rc === "52")
    throw new InvalidParams(error.data.message, error.data);
  if (rc === "41") throw new InvalidSignature(error.data.message, error.data);
  if (rc === "44")
    throw new InsufficientBalance(error.data.message, error.data);
  ...
  if (rc === "55" || rc === "56")
    throw new ProductUnavailable(error.data.message, error.data);
